...
// function used by mapply for likelihood
...
// [[Rcpp::export]]
double like(NumericVector outcome, NumericMatrix dat, NumericVector disp, NumericVector par){
  int np = dat.ncol();
  ...
  for (int i=1;i<iter;++i) { 
    ...
    for (int k=0;k<nc;++k){
      if (k<nw+1){
        ...
        ratio = exp(like(outcome1,cov,dpind,newpar) - like(outcome1,cov,dpind,oldpar));
        ...
        if (newpar[k] < 100) {
          ratio = (newpar[k]/oldpar[k])*exp(like(outcome1,cov,dpind,newpar) - like(outcome1,cov,dpind,oldpar));
        } else {
          ratio = 0;
        }
        decision = (ratio>1)?1:ratio;
    ...
    NumericVector now = back1/weights;
    backc[seq(nd,nd+maxdelay-1)] = now;
    back(i,_) = backc;
