...
simfun <- function(trial,rt,maxdelay,si){
  ...
  d <- out$dat
  d1 <- d%>%filter(report>10)%>%arrange(report)
  d1 <- d1%>%mutate(weekend=ifelse(report%%7==0|report%%7==6,1,0))
  d1 <- d1%>%mutate(delay=report-onset,minday=min(report))
  d1 <- d1%>%mutate(report=report-minday+1,week=ceiling(report/7))
  d2 <- d%>%filter(report>20)%>%arrange(report)
  d2 <- d2%>%mutate(weekend=ifelse(report%%7==0|report%%7==6,1,0))
  d2 <- d2%>%mutate(delay=report-onset,minday=min(report))
  d2 <- d2%>%mutate(report=report-minday+1,week=ceiling(report/7))
  d3 <- d%>%filter(report>30)%>%arrange(report)
  d3 <- d3%>%mutate(weekend=ifelse(report%%7==0|report%%7==6,1,0))
  d3 <- d3%>%mutate(delay=report-onset,minday=min(report))
  d3 <- d3%>%mutate(report=report-minday+1,week=ceiling(report/7))
  d4 <- d%>%filter(report>40)%>%arrange(report)
  d4 <- d4%>%mutate(weekend=ifelse(report%%7==0|report%%7==6,1,0))
  d4 <- d4%>%mutate(delay=report-onset,minday=min(report))
  d4 <- d4%>%mutate(report=report-minday+1,week=ceiling(report/7))
  epic <- out$epic
  repc <- out$repc
  minday <- c(unique(d1$minday),unique(d2$minday),unique(d3$minday),unique(d4$minday))
  out1 <- backnow(outcome=d1$delay,days=d1$report,week=d1$week,weekend=d1$weekend,iter=21000,sigma=0.2,maxdelay=20,si=si,size=6)
  est1 <- apply(out1$Back[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  estr1 <- apply(out1$R[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  out2 <- backnow(outcome=d2$delay,days=d2$report,week=d2$week,weekend=d2$weekend,iter=21000,sigma=0.2,maxdelay=20,si=si,size=6)
  est2 <- apply(out2$Back[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  estr2 <- apply(out2$R[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm=TRUE))
  out3 <- backnow(outcome=d3$delay,days=d3$report,week=d3$week,weekend=d3$weekend,iter=21000,sigma=0.2,maxdelay=20,si=si,size=6)
  est3 <- apply(out3$Back[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  estr3 <- apply(out3$R[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  out4 <- backnow(outcome=d4$delay,days=d4$report,week=d4$week,weekend=d4$weekend,iter=21000,sigma=0.2,maxdelay=20,si=si,size=6)
  est4 <- apply(out4$Back[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  estr4 <- apply(out4$R[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  epicr <- getr(epic,si,6)
  repcr <- getr(repc,si,6)
  result <- list(est1=est1,est2=est2,est3=est3,est4=est4,estr1=estr1,estr2=estr2,estr3=estr3,estr4=estr4,epic=epic,epicr=epicr,
                 repc=repc,repcr=repcr,minday=minday)
  return(result)
}
...
saveRDS(out1,"sce1_ss.rds")
saveRDS(out2,"sce2_ss.rds")
saveRDS(out3,"sce3_ss.rds")
saveRDS(out4,"sce4_ss.rds")
saveRDS(out5,"sce5_ss.rds")
saveRDS(out6,"sce6_ss.rds")

## for chopping off tails 
simfun <- function(trial,rt,maxdelay,si){
  out <- simdata(100,1.8,rt,3,9,maxdelay,60)
  d <- out$dat
  d1 <- d%>%filter(report<=28)%>%arrange(report)
  d1 <- d1%>%mutate(weekend=ifelse(report%%7==0|report%%7==6,1,0))
  d1 <- d1%>%mutate(delay=report-onset,minday=min(report))
  d1 <- d1%>%mutate(report=report-minday+1,week=ceiling(report/7))
  d2 <- d%>%filter(report<=38)%>%arrange(report)
  d2 <- d2%>%mutate(weekend=ifelse(report%%7==0|report%%7==6,1,0))
  d2 <- d2%>%mutate(delay=report-onset,minday=min(report))
  d2 <- d2%>%mutate(report=report-minday+1,week=ceiling(report/7))
  epic <- out$epic
  repc <- out$repc
  minday <- unique(d1$minday)
  out1 <- backnow(outcome=d1$delay,days=d1$report,week=d1$week,weekend=d1$weekend,iter=21000,sigma=0.2,maxdelay=20,si=si,size=6)
  est1 <- apply(out1$Back[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  estr1 <- apply(out1$R[seq(1001,21000,by=2),],2,function(x) quantile(x,probs = c(0.025,0.5,0.975),na.rm = TRUE))
  out2 <- backnow(outcome=d2$delay,days=d2$report,week=d2$week,weekend=d2$weekend,iter=21000,sigma=0.2,maxdelay=20,si=si,size=6)
...
saveRDS(out1,"sce1_ss1.rds")
saveRDS(out2,"sce2_ss1.rds")
saveRDS(out3,"sce3_ss1.rds")
saveRDS(out4,"sce4_ss1.rds")
saveRDS(out5,"sce5_ss1.rds")
saveRDS(out6,"sce6_ss1.rds")
