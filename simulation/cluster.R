...
cl <- makeCluster(P)
clusterEvalQ(cl, {library(tidyverse);library(backnow);library(coda)})
...
simfun <- function(trial,rt,maxdelay,si){
  ...
  out1 <- backnow(outcome=d$delay,days=d$report,week=d$week,weekend=d$weekend,iter=21000,sigma=0.2,maxdelay=20,si=si,size=6)
  ...
  out2 <- backnow(outcome=d$delay,days=d$report,week=d$week,weekend=d$weekend,iter=21000,sigma=0.2,maxdelay=20,si=si,size=6,cd=31-minday)
  back2 <- out2$Back[seq(1001,21000,by=2),]
  gback2 <- geweke.diag(back2)$z
  gback2[is.nan(gback2)] <- 0
  gb2 <- sum(abs(gback2)>1.96)/length(gback2)
  est2 <- apply(back2,2,function(x) quantile(x,probs = c(0.025,0.5,0.975)))
  r2 <- out2$R[seq(1001,21000,by=2),]
  gr2 <- geweke.diag(r2)$z
  gr2[is.nan(gr2)] <- 0
  gr2 <- sum(abs(gr2)>1.96)/length(gr2)
  estr2 <- apply(r2,2,function(x) quantile(x,probs = c(0.025,0.5,0.975)))
  epicr <- getr(epic,si,6)
  repcr <- getr(repc,si,6)
  geweke <- c(gb1,gr1,gb2,gr2)
  result <- list(est1=est1,est2=est2,estr1=estr1,estr2=estr2,epic=epic,epicr=epicr,repc=repc,repcr=repcr,minday=minday,geweke=geweke)
  return(result)
}
